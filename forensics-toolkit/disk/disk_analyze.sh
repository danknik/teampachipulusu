...
usage() {
    echo "Usage: $(basename "$0") <disk_image> [options]"
    echo ""
    echo "  disk_image         Path to the disk image file"
    echo ""
    echo "Options:"
    echo "  --skip-recovery    Skip deleted file recovery (faster)"
    echo "  --skip-stego       Skip steganography scanning"
    echo "  --skip-timeline    Skip filesystem timeline generation"
    echo "  --dry-run          Print commands without executing"
    echo "  --help, -h         Show this help"
    echo ""
    echo "Master orchestrator â€” runs all disk analysis scripts in sequence."
    exit 1
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --skip-stego)     SKIP_STEGO=true; shift ;;
...
[[ -z "$IMAGE" ]] && usage
[[ ! -f "$IMAGE" ]] && { error "Disk image not found: ${IMAGE}"; exit 1; }

RUN_OUT=$(make_output_dir disk)

IMAGE_SIZE=$(stat -c%s "$IMAGE" 2>/dev/null || stat -f%z "$IMAGE" 2>/dev/null || echo "unknown")

banner "DISK FORENSICS â€” $(basename "$IMAGE")"
echo -e "  Image:  ${IMAGE} (${IMAGE_SIZE} bytes)"
...
run_script "carve" "${SCRIPT_DIR}/carve.sh" "$IMAGE" "${RUN_OUT}/carved"

if [[ "$SKIP_TIMELINE" == false ]]; then
    run_script "timeline" "${SCRIPT_DIR}/timeline.sh" "$IMAGE" "${RUN_OUT}/timeline"
else
    info "Skipping timeline generation (--skip-timeline)"
fi

if [[ "$SKIP_RECOVERY" == false ]]; then
    run_script "recover_deleted" "${SCRIPT_DIR}/recover_deleted.sh" "$IMAGE" "${RUN_OUT}/recovered"
else
    info "Skipping deleted file recovery (--skip-recovery)"
fi

EXTRACTED_DIRS=()
[[ -d "${RUN_OUT}/carved" ]] && EXTRACTED_DIRS+=("${RUN_OUT}/carved")
[[ -d "${RUN_OUT}/recovered" ]] && EXTRACTED_DIRS+=("${RUN_OUT}/recovered")

if [[ ${#EXTRACTED_DIRS[@]} -gt 0 ]]; then
    for edir in "${EXTRACTED_DIRS[@]}"; do
        run_script "metadata_sweep_$(basename "$edir")" "${SCRIPT_DIR}/metadata_sweep.sh" \
            "$edir" "${RUN_OUT}/metadata_$(basename "$edir")"
    done
else
    info "No extracted files for metadata sweep"
fi

if [[ "$SKIP_STEGO" == false ]]; then
    for edir in "${EXTRACTED_DIRS[@]}"; do
        IMG_COUNT=$(find "$edir" -type f \( -iname "*.png" -o -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.bmp" -o -iname "*.gif" -o -iname "*.wav" \) 2>/dev/null | wc -l)
        if [[ $IMG_COUNT -gt 0 ]]; then
            run_script "stego_scan_$(basename "$edir")" "${SCRIPT_DIR}/stego_scan.sh" \
                "$edir" "${RUN_OUT}/stego_$(basename "$edir")"
        fi
    done
else
    info "Skipping stego scan (--skip-stego)"
fi
...
echo ""
section "All flags found"
grep -rEho "${FLAG_PATTERNS}" "$RUN_OUT" 2>/dev/null | sort -u | while IFS= read -r flag; do
    highlight "${flag}"
