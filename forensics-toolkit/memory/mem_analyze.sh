...
usage() {
    echo "Usage: $(basename "$0") <memory_dump> [options]"
    echo ""
    echo "  memory_dump        Path to the RAM dump file"
    echo ""
    echo "Options:"
    echo "  --skip-filedump    Skip file object extraction (faster)"
    echo "  --skip-strings     Skip full string extraction (faster)"
    echo "  --dry-run          Print commands without executing"
    echo "  --help, -h         Show this help"
    echo ""
    echo "Master orchestrator â€” runs all memory analysis scripts in sequence."
    exit 1
}
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --skip-filedump) SKIP_FILEDUMP=true; shift ;;
        --skip-strings)  SKIP_STRINGS=true; shift ;;
...
RUN_OUT=$(make_output_dir memory)
VOL_OUT="${RUN_OUT}/vol3_plugins"

DUMP_SIZE=$(stat -c%s "$DUMP" 2>/dev/null || stat -f%z "$DUMP" 2>/dev/null || echo "unknown")

banner "MEMORY FORENSICS â€” $(basename "$DUMP")"
echo -e "  Dump:   ${DUMP} (${DUMP_SIZE} bytes)"
...
run_script "vol3_runner" "${SCRIPT_DIR}/vol3_runner.sh" "$DUMP" "$VOL_OUT"

run_script "proc_dumper" "${SCRIPT_DIR}/proc_dumper.sh" "$DUMP" "$VOL_OUT" "${RUN_OUT}/proc_dumps"

if [[ "$SKIP_FILEDUMP" == false ]]; then
    run_script "file_extractor" "${SCRIPT_DIR}/file_extractor.sh" "$DUMP" "$VOL_OUT" "${RUN_OUT}/extracted_files"
else
    info "Skipping file extraction (--skip-filedump)"
fi

if [[ "$SKIP_STRINGS" == false ]]; then
    run_script "mem_strings" "${SCRIPT_DIR}/mem_strings.sh" "$DUMP" "${RUN_OUT}/strings"
else
    info "Skipping string extraction (--skip-strings)"
fi
...
banner "MEMORY ANALYSIS COMPLETE"
...
find "$RUN_OUT" -type f -name "*.txt" -exec grep -lEo "${FLAG_PATTERNS}" {} \; 2>/dev/null \
    | while IFS= read -r f; do
        grep -Eo "${FLAG_PATTERNS}" "$f" 2>/dev/null | while IFS= read -r flag; do
            highlight "${flag}  (in $(basename "$f"))"
        done
    done | sort -u || info "No flags found in this run"
